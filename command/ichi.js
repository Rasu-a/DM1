...
if (!('templateLocation' in settings)) settings.templateLocation = true
...
if (budy.match('https://youtu')) {
  ...
  ichi.sendMessage(m.chat, { document: { url: media.dl_link }, mimetype: 'video/mp4', fileName: `${media.title}.mp4` }, { quoted: m })
  global.db.data.users[m.sender].limit -= 1
} else if (budy.match('https://www.tiktok.com')) {
  if (!isPrem && global.db.data.users[m.sender].limit < 1) return m.reply(global.limitEnd)
  ...
  instagramdl(budy).then(async res => {
    ...
    for (let { url, type } of json) {
      ichi.sendFile(m.chat, url, global.ownerName, m)
    }
  })
  global.db.data.users[m.sender].limit -= 5
  }
...
case 'setmenu': {
  ...
  set.templateImage = false
  set.listMessage = false
  set.templateLocation = true
  m.reply(mess.done)
  } else {
  let buttonsset = [
  { buttonId: `setmenu image`, buttonText: { displayText: 'Setmenu Image' }, type: 1 },
  { buttonId: `setmenu list`, buttonText: { displayText: 'Setmenu List' }, type: 1 },
  { buttonId: `setmenu location`, buttonText: { displayText: 'Setmenu Location' }, type: 1 }
  ]
  ichi.sendButtonText(m.chat, buttonsset, `Silahkan Klik Button Dibawah, Jika Button Tidak Muncul Ketik Manual Saja`, global.ownerName, m)
  }
  }
  break
case 'react': {
...
case 'git': case 'gitclone': {
  ...
  ichi.sendMessage(m.chat, { document: { url: url }, fileName: filename+'.zip', mimetype: 'application/zip' }, { quoted: m })
  global.db.data.users[m.sender].limit -= 1
  }
  break
...
⏳ Tunggu Proses Pengiriman Media`
  ichi.sendButtonLoc(m.chat, caption, global.ownerName, `${media.thumb}`)
  ichi.sendMessage(m.chat, { document: { url: media.dl_link }, mimetype: 'video/mp4', fileName: `${media.title}.mp4` }, { quoted: m })
  global.db.data.users[m.sender].limit -= 1
  }
  break
...
⏳ Tunggu Proses Pengiriman Media`
  ichi.sendButtonLoc(m.chat, caption, global.ownerName, `${media.thumb}`)
  ichi.sendMessage(m.chat, { audio: { url: media.dl_link }, mimetype: 'audio/mpeg', ptt: true }, { quoted: m })
  global.db.data.users[m.sender].limit -= 1
  }
  break
...
🔗 Url : ${i.url}\n\n─────────────────\n\n`
  ...
  ichi.sendButtonLoc(m.chat, teks, global.ownerName, `${search.all[0].thumbnail}`)
  global.db.data.users[m.sender].limit -= 1
  }
  break
case 'play':
...
*🔗 Url :* ${anu.url}`
  var butyt = [{urlButton: {displayText: 'Go To Youtube 📺',url: `${anu.url}`}}, {"quickReplyButton": {"displayText": "🎧 Audio","id": `ytmp3 ${anu.url}`},}, {"quickReplyButton": {"displayText": "🎦 Video","id": `ytmp4 ${anu.url}`}}]
  ichi.sendButtonLoc(m.chat, caption, global.ownerName, `${anu.thumbnail}`, butyt)
  global.db.data.users[m.sender].limit -= 1
  break
*/
...
case 'menu': case 'help': case 'command': {
  ...
  ichi.sendButtonImg(m.chat, menuu+simplemenu(prefix), global.ownerName, global.thumb, butmenu, global.thumb)
  } else if (set.templateLocation) {
  ichi.sendButtonLoc(m.chat, menuu+simplemenu(prefix), global.ownerName, global.thumb, butmenu)
  }
  }
  break
...
case 'searchmenu': case 'searchcmd': {
  ...
  ichi.sendButtonImg(m.chat, menu+searchmenu(prefix), global.ownerName, global.thumb, butmenu, global.thumb)
  }
  break
...
case 'gamemenu': case 'gamecmd': {
  ...
  ichi.sendButtonImg(m.chat, menu+gamemenu(prefix), global.ownerName, global.thumb, butmenu, global.thumb)
  }
  break
case 'textpromenu': case 'textprocmd': {
  var { textpromenu } = require('../storage/confat.js')
  ichi.sendButtonImg(m.chat, menu+textpromenu(prefix), global.ownerName, global.thumb, butmenu, global.thumb)
  }
  break
case 'allmenu': case 'allcmd': {
  let { textpromenu, gamemenu, ownermenu, groupmenu, storemenu, rpgmenu, makermenu, downloadmenu, ephotomenu, photooxymenu, islamimenu, randommenu, searchmenu, gachamenu, asupanmenu, randomimagemenu, randomanimemenu, wallpapermenu, nsfwmenu, othermenu } = require('../storage/confat.js')
  ichi.sendButtonLoc(m.chat, menuu+ownermenu(prefix)+'\n'+groupmenu(prefix)+'\n'+storemenu(prefix)+'\n'+rpgmenu(prefix)+'\n'+gamemenu(prefix)+'\n'+makermenu(prefix)+'\n'+downloadmenu(prefix)+'\n'+textpromenu(prefix)+'\n'+ephotomenu(prefix)+'\n'+photooxymenu(prefix)+'\n'+islamimenu(prefix)+'\n'+randommenu(prefix)+'\n'+searchmenu(prefix)+'\n'+gachamenu(prefix)+'\n'+asupanmenu(prefix)+'\n'+randomimagemenu(prefix)+'\n'+randomanimemenu(prefix)+'\n'+wallpapermenu(prefix)+'\n'+nsfwmenu(prefix)+'\n'+othermenu(prefix), global.ownerName, global.thumb, butmenu)
  }
  break
...
case 'infocovid': case 'covid':
  if (!isPrem && global.db.data.users[m.sender].limit < 1) return m.reply(global.limitEnd)
  if (!text) return reply(`Masukkan Nama Negara\n\nExample: ${prefix + command} indonesia`)
  let covid = await fetchJson('https://covid19.mathdro.id/api/countries/'+`${text}`)
  let covid2 = `🌍 Negara : ${text}
...
case 'kbbi':
  ...
  m.reply(`*🔑 Kata Kunci :* ${data.result.lema}\n\n*🚩Arti :* ${data.result.arti}\n`)
  global.db.data.users[m.sender].limit -= 1
  break
...
case 'tiktokaudio': case 'tiktokmp3': case 'ttaudio': case 'ttmp3': {
  ...
  ichi.sendMessage(m.chat, { audio: { url: url }, mimetype: 'audio/mpeg', fileName: 'tiktok.mp3' }, { quoted: m })
  global.db.data.users[m.sender].limit -= 5
  }
  break
...
case 'jadibot': {
  imgbot = await getBuffer ('https://telegra.ph/file/14a2fca7190b12db28557.jpg')
  var botmulu = `Jadibot Tinggal Scan (Permanen)

Harga: 30k
Free Scan 5x, Bebas Script Siapapun

Sistem: Proses - Bot On - Bayar
  `
  ichi.sendCatalog(m.chat, 'Bosan Jadibot Numpang Mulu?', botmulu, global.ownername, global.ownername, global.webme, global.Rowner, imgbot, {quoted: m})
  }
  break
